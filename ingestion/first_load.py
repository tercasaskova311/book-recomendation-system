...
def load_books_from_kaggle(csv_path):
    ...
    for _, row in tqdm(df_valid.iterrows(), total=len(df_valid), desc="Preparing"):
        try:
    ...
    print(f"✅ Loaded {inserted:,} books into database")
...
def load_users(csv_path):
    ...
    inserted = cursor.rowcount
    print(f"✅ Loaded {inserted:,} users into database")
...
def load_ratings(csv_path):
    ...
    inserted = cursor.rowcount
    print(f"✅ Loaded {inserted:,} ratings into database")
